...
from SegEarthR1.model.mask_decoder.Mask2Former_Simplify.utils.misc import is_dist_avail_and_initialized, \
    nested_tensor_from_tensor_list
from SegEarthR1.model.mask_decoder.Mask2Former_Simplify.utils.point_features import point_sample, \
    get_uncertain_point_coords_with_randomness
from SegEarthR1.model.mask_decoder.Mask2Former_Simplify.utils.matcher import HungarianMatcher, batch_dice_loss_jit, \
    batch_sigmoid_ce_loss_jit, batch_sigmoid_focal_loss
from SegEarthR1.model.mask_decoder.Mask2Former_Simplify.utils.criterion import SetCriterion
...
class SegEarthR1_criterion(nn.Module): # 计算loss的

    def __init__(self, matcher, losses, num_points, oversample_ratio, importance_sample_ratio, device):
        super().__init__()
        self.matcher = matcher # hungarian_matcher_PSALM
        self.losses = losses # ["SEG_labels", "class_name_labels", "masks", "region_labels"]
        self.num_points = num_points # 12544 112 * 112
        self.oversample_ratio = oversample_ratio # 3.0
        self.importance_sample_ratio = importance_sample_ratio # 0.75
        self.device = device 
        self.pos_weight = torch.tensor([99.0]) # 什么玩意
        self.SEG_temp = 0.07 # 什么玩意
        self.CLASS_temp = 0.07 # 什么玩意

    def loss_labels(self, outputs, targets, indices):
        pass

    def loss_SEG_labels(self, outputs, targets, indices, num_masks): # 这个损失好好看看
        assert "pred_SEG_logits" in outputs
        # src_logits [batch_size, num_query, 1]
        pred_SEG_logits = outputs['pred_SEG_logits'] # pred_SEG_logits: [batch_size, 100, 1]
        if pred_SEG_logits is None:
            return {"loss_SEG_class": None}
        src_logits = pred_SEG_logits.float()
        src_logits = src_logits # 这是啥啊
        target_query = torch.zeros_like(src_logits).to(src_logits.device) # target_query: [batch_size, 100, 1] all zeros
        ...
        loss_SEG_class = loss_func(src_logits, target_query) # 用于预测哪个query的
        # loss_SEG_class = -pos_weight * (target_query * torch.log(src_logits)) - neg_weight * ((1 - target_query) * torch.log(1 - src_logits)) # 加入mask
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        src_idx = self._get_src_permutation_idx(indices) # [torch.tensor([0, 1]), torch.tensor([10, 54])
        tgt_idx = self._get_tgt_permutation_idx(indices) # [torch.tensor([0, 1]), torch.tensor([0, 0])]
        src_masks = outputs["pred_masks"] # src_masks: [batch_size, 100, 256, 256]
        src_masks = src_masks[src_idx] # 选了第10和54个 [batch_size, 256, 256]
        masks = [t["masks"] for t in targets] # masks: [[1, 1024, 1024], [1, 1024, 1024]]
        # TODO use valid to mask invalid areas due to padding in loss
        target_masks, valid = nested_tensor_from_tensor_list(masks).decompose()
        target_masks = target_masks.to(src_masks)
        target_masks = target_masks[tgt_idx] # target_masks: [batch_size, 1024, 1024]
        # target_masks = target_masks[src_idx]
        # No need to upsample predictions as we are using normalized coordinates :)
        # N x 1 x H x W
        src_masks = src_masks[:, None] # src_masks: [batch_size, 256, 256] -> [batch_size, 1, 256, 256]
        target_masks = target_masks[:, None] # target_masks: [batch_size, 1024, 1024] -> [batch_size, 1, 1024, 1924]
    ...
    def _get_src_permutation_idx(self, indices): # indices: [[10, 0], [54, 0]]
        # permute predictions following indices
        batch_idx = torch.cat([torch.full_like(src, i) for i, (src, _) in enumerate(indices)]) # batch_idx: torch.tensor([0, 1])
        src_idx = torch.cat([src for (src, _) in indices]) # src_idx: torch.tensor([10, 54])
        return batch_idx, src_idx

    def _get_tgt_permutation_idx(self, indices):
        # permute targets following indices
        batch_idx = torch.cat([torch.full_like(tgt, i) for i, (_, tgt) in enumerate(indices)]) # batch_idx: torch.tensor([0, 1])
        tgt_idx = torch.cat([tgt for (_, tgt) in indices]) # tgt_idx: torch.tensor([0, 0])
    ...
    def forward(self, outputs, targets):
        outputs_without_aux = {k: v for k, v in outputs.items() if k != "aux_outputs"} # outputs_without_aux: {"pred_SEG_logits": [batch_size, 100, 1], "pred_class_name_logits": None, "pred_region_logits": None, "pred_masks": [batch_size, 100, 256, 256]}

        # Retrieve the matching between the outputs of the last layer and the targets
        indices = self.matcher(outputs_without_aux, targets) # [[10, 0], [54, 0]]
