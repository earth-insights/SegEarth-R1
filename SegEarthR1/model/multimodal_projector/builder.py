...
class BasicBlock(nn.Module):
    ...
    def forward(self, x): # x: [bs, 1024, 32, 32]
        residual = x

        out = self.conv1(x) # [bs, 1024, 32, 32] -> [bs, 2048, 16, 16]
        out = self.bn1(out) # no change
        out = self.relu(out) # no change

        out = self.conv2(out) # no change
        if not self.with_dcn:
            out = self.conv2(out)
        elif self.with_modulated_dcn:
            offset_mask = self.conv2_offset(out)
...
class ResNetSwin(nn.Module):
    ...
    def forward(self, x): # [bs, 1024, 32, 32]

        x = self.layer1(x) # [bs, 2048, 16, 16]

        x = x.reshape(x.shape[0],x.shape[1],-1) # [bs, 2048, 256]
        x = x.permute(0,2,1)
...
class SparseConv_1(nn.Module):
    def __init__(self, c1=1024, c2=2048, patch_size=3, stride=2, size=16):
        super().__init__()
        self.proj1 = nn.Conv2d(c1, c2, patch_size, stride, patch_size//2)    # padding=(ps[0]//2, ps[1]//2)
        self.norm1 = nn.LayerNorm([c2, size, size])
        
        self.proj2 = nn.Conv2d(c2, c2, patch_size, stride, patch_size//2)    # padding=(ps[0]//2, ps[1]//2)
        self.norm2 = nn.LayerNorm([c2, size//2, size//2])
...
class SparseConv_3(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.layer1 = self._make_layer(
            BasicBlock, self.inplanes, 1, stride=2, dcn=dcn)
        self.layer2 = self._make_layer(
            BasicBlock, out_dim, 1, stride=2, dcn=dcn)
...
class SparseConv_s8(nn.Module):
    ...
    def forward(self, x): # 输入[bs, 1024, 32, 32]

        x = self.layer1(x) # [bs, 1024, 16, 16]
        x = self.layer2(x) # [bs, 1024, 8, 8]
        x = self.layer3(x) # [bs, 2048, 4, 4]

        x = x.reshape(x.shape[0],x.shape[1],-1) # [bs, 2048, 16]
        x = x.permute(0,2,1)
        x = self.fc(x)

        return x

class SparseConv_s8v2(nn.Module):
    def __init__(self, c1=1024, c2=2048, patch_size=3, stride=2, size=16):
        super().__init__()
        self.proj1 = nn.Conv2d(c1, c2, patch_size, stride, patch_size//2)
        self.norm1 = nn.LayerNorm([c2, size, size])
        
        self.proj2 = nn.Conv2d(c2, c2, patch_size, stride, patch_size//2)
        self.norm2 = nn.LayerNorm([c2, size//2, size//2])
        
        self.proj3 = nn.Conv2d(c2, c2, patch_size, stride, patch_size//2) 
        self.norm3 = nn.LayerNorm([c2, size//4, size//4])
...
class SparseConv_2(nn.Module):
    def __init__(self, c1=1024, c2=512, patch_size=3, stride=2, size=16):
        super().__init__()
        self.proj = nn.Conv2d(c1, c2, patch_size, stride, patch_size//2)
        self.norm = nn.LayerNorm([c2, size, size])
        
        # self.fc = nn.Conv2d(c2 * 4, c2 * 4, 1, 1)
        self.fc =  nn.Linear(c2 * 4, c2 * 4)
    ...
    def forward(self, x):
        ...
        if True:
            ...
            x = x.permute(0, 3, 5, 1, 2, 4)  # [B, 2, 2, C, H/2, W/2]
            x = x.reshape(B, 4*C, H//2 * W//2).permute(0, 2, 1).contiguous()
        else:
            x = self.unfold(x)  # [B, C*2 * 2, (H/2/2)*(W/2/2)]
...
def build_vision_projector(config, delay_load=False, **kwargs):
    ...
    if projector_type == 'SparseConv_1':
        out_dim = getattr(config,'projector_outdim',4096) # 2048
        input_dim = getattr(config,'mm_input_embeds',1024) # 1024
        return SparseConv_1(input_dim, out_dim, size=16)
    if projector_type == 'SparseConv_2':
        out_dim = 512
        input_dim = getattr(config,'mm_input_embeds',1024) # 1024
        return SparseConv_2(input_dim, out_dim, size=16)
    if projector_type == 'SparseConv_3':
        out_dim = getattr(config,'projector_outdim',4096) # 2048
        input_dim = getattr(config,'mm_input_embeds',1024) # 1024
        return SparseConv_3(input_dim=input_dim,out_dim=out_dim)
    if projector_type == 'SparseConv_s8':
        out_dim = getattr(config,'projector_outdim',4096) # 2048
        input_dim = getattr(config,'mm_input_embeds',1024) # 1024
        return SparseConv_s8(input_dim=input_dim, out_dim=out_dim)
    if projector_type == 'SparseConv_s8v2':
        out_dim = getattr(config,'projector_outdim',4096) # 2048
        input_dim = getattr(config,'mm_input_embeds',1024) # 1024
        return SparseConv_s8v2(input_dim, out_dim, size=16)
