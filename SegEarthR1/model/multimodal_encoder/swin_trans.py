...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        patch_size = to_2tuple(patch_size) # 变成元组 4 -> (4, 4)
        ...
        if norm_layer is not None:
            self.norm = norm_layer(embed_dim) # nn.LayerNorm
        else:
            self.norm = None
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.pretrain_img_size = pretrain_img_size # 224
        self.num_layers = len(depths) #[2, 2, 18, 2] 4
        self.embed_dim = embed_dim # 128
        self.ape = ape # False 这是个什么参数?
        ...
        self.patch_embed = PatchEmbed(
            ...
            norm_layer=norm_layer if self.patch_norm else None) # 4  # 3  # 128  # nn.LayerNorm  # 
        ...
        num_features = [int(embed_dim * 2 ** i) for i in range(self.num_layers)] # [128, 256, 512, 1024] 
        self.num_features = num_features

        # add a norm layer for each output
        for i_layer in out_indices:
            layer = norm_layer(num_features[i_layer]) # nn.LayerNorm(dim)
...
if __name__ == '__main__':
    model = build_swin_b('/home/emzhang/code/Swin-Transformer-Semantic-Segmentation/model_final_base.pkl')
