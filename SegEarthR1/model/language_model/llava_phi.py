...
from SegEarthR1.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, \
...
from SegEarthR1.model.mask_decoder.mask_criterion.pretrain_criterion import SegEarthR1_criterion, hungarian_matcher_PSALM
from transformers import PhiModel, PhiForCausalLM, PhiConfig

from SegEarthR1.model.language_model.projector import D_Projector
...
class SegEarthR1Model(LlavaMetaModel, PhiModel): 
    config_class = LlavaConfig

    def __init__(self, config: PhiConfig, mask_decoder_cfg=None):
        super(SegEarthR1Model, self).__init__(config)
...
class SegEarthR1(PhiForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaConfig

    def __init__(self, config, mask_decoder_cfg=None, add_cross_attn=True, cross_attn_index=None, use_seg_query=False):
        super(SegEarthR1, self).__init__(config)

        self.model = SegEarthR1Model(config, mask_decoder_cfg)
    ...
    def mask_decoder_training_init(self, cfg):
        ...
        self.criterion = SegEarthR1_criterion(
            matcher=matcher,
            losses=losses,
...
AutoModelForCausalLM.register(LlavaConfig, SegEarthR1)
