...
from segearth_r1.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class LlavaMetaForCausalLM(ABC):
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer): # 写的啥啊 if else if ?
        ...
        if model_args.mm_use_im_start_end:  # False
            ...
            if model_args.tune_mm_mlp_adapter:   # 当微调mlp时, input_embeddings 不更新梯度?
                for p in self.get_input_embeddings().parameters():
                    p.requires_grad = False
                for p in self.get_output_embeddings().parameters():
                    p.requires_grad = False
